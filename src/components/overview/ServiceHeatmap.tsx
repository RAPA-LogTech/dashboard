    ...
    >
      ...
      {services.length === 0 ? (
        ...
        <Box sx={{ overflowX: 'auto' }}>
          {/* Header row */}
          <Box sx={{ display: 'grid', gridTemplateColumns: `120px repeat(${envs.length}, 1fr)`, gap: 0.5, mb: 0.5 }}>
            <Box />
            {envs.map(env => (
              <Typography key={env} variant="caption" sx={{ fontWeight: 700, color: 'text.secondary', textAlign: 'center', letterSpacing: 0.5 }}>
                {env.toUpperCase()}
              </Typography>
            ))}
          </Box>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.5 }}>
            {serviceNames.map(svc => (
              <Box key={svc} sx={{ display: 'grid', gridTemplateColumns: `120px repeat(${envs.length}, 1fr)`, gap: 0.5, alignItems: 'center' }}>
                <Typography variant="caption" sx={{ fontWeight: 600, color: 'text.secondary', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', pr: 1 }}>
                  {svc}
                </Typography>
                {envs.map(env => {
                  ...
                  if (!cell) {
                    return (
                      <Box key={env} sx={{ height: 36, borderRadius: 1, bgcolor: 'action.hover', border: '1px solid', borderColor: 'divider' }} />
                    )
                  }
                  const colors = getHeatColor(cell.error_rate)
                  return (
                    <Tooltip key={env} title={`${svc} · ${env} · ${cell.error_rate.toFixed(2)}% error rate`} arrow>
                      <Box
                        ...
                        sx={{
                          ...
                          '&:hover': { border: `1px solid ${colors.border}`, transform: 'scale(1.04)' },
                        }}
                      >
