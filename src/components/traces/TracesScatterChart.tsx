...
export default function TracesScatterChart({
  traces, isLiveEnabled, streamStatus, onLiveChange, onSelectTrace, selectedTraceId,
}: Props) {
  ...
  const chartData = useMemo(() => {
    return traces
      ...
      .map(t => {
        ...
        return {
          id: t.id, service: t.service, operation: t.operation,
  ...
  return (
    <Paper variant="outlined" sx={{ p: 2, borderColor: 'divider', bgcolor: 'background.paper' }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 1.5 }}>
        <Box>
          <Typography variant="body2" sx={{ fontWeight: 700 }}>Trace Timeline</Typography>
          <Typography variant="caption" sx={{ color: 'text.secondary' }}>Duration vs Time · last 10 minutes</Typography>
        </Box>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
          ...
          <LiveButton value={isLiveEnabled} isStreaming={streamStatus === 'live'} onChange={onLiveChange} />
        </Box>
      </Box>

      <Box sx={{ height: 240 }}>
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 8, right: 16, bottom: 8, left: 0 }} onMouseLeave={() => setHoveredId(null)}>
            <CartesianGrid stroke={theme.palette.divider} strokeDasharray="3 3" />
            <XAxis type="number" dataKey="startTime" domain={xDomain} ticks={xTicks}
              tickFormatter={formatTime} tick={{ fill: theme.palette.text.secondary, fontSize: 10 }}
              tickLine={false} axisLine={{ stroke: theme.palette.divider }} minTickGap={24} />
            <YAxis type="number" dataKey="duration" domain={yDomain}
              tickFormatter={formatDuration} tick={{ fill: theme.palette.text.secondary, fontSize: 10 }}
              tickLine={false} axisLine={{ stroke: theme.palette.divider }} width={52} />
            ...
            <Tooltip
              ...
              content={({ active, payload }) => {
                ...
                return (
                  <Box sx={{ bgcolor: 'background.paper', border: '1px solid', borderColor: 'divider', borderRadius: 1, p: 1.25, minWidth: 200 }}>
                    <Typography variant="caption" sx={{ fontWeight: 700, display: 'block', mb: 0.5 }}>
                      {item.service}: {item.operation}
                    </Typography>
                    <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block', fontFamily: 'monospace', fontSize: 10, mb: 0.5 }}>{item.id}</Typography>
                    <Box sx={{ display: 'flex', flexDirection: 'column', gap: 0.25 }}>
                      <Typography variant="caption" sx={{ color: 'text.secondary' }}>Duration: <Box component="span" sx={{ color: 'text.primary', fontWeight: 700 }}>{formatDuration(item.duration)}</Box></Typography>
                      <Typography variant="caption" sx={{ color: 'text.secondary' }}>Spans: <Box component="span" sx={{ color: 'text.primary', fontWeight: 700 }}>{item.spanCount}</Box> ({item.errorSpanCount} errors)</Typography>
                      {item.statusCode && <Typography variant="caption" sx={{ color: 'text.secondary' }}>HTTP: <Box component="span" sx={{ color: 'text.primary', fontWeight: 700 }}>{item.statusCode}</Box></Typography>}
                    </Box>
                  </Box>
                )
              }}
            ...
            <Scatter data={chartData} isAnimationActive={false}
              ...
              onClick={(p: any) => { if (p?.id) onSelectTrace(selectedTraceId === p.id ? null : p.id) }}
            >
              {chartData.map(p => {
      ...
      <Box sx={{ display: 'flex', gap: 2, mt: 1, justifyContent: 'center' }}>
        {[{ label: 'OK', color: '#60a5fa' }, { label: 'Error', color: '#f87171' }, { label: 'Slow', color: '#fbbf24' }].map(item => (
          <Box key={item.label} sx={{ display: 'flex', alignItems: 'center', gap: 0.5 }}>
            <Box sx={{ width: 8, height: 8, borderRadius: '50%', bgcolor: item.color }} />
            <Typography variant="caption" sx={{ fontSize: 10, color: 'text.secondary' }}>{item.label}</Typography>
          </Box>
        ))}
      </Box>
    </Paper>
