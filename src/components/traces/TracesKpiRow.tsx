...
function KpiCard({ label, value, sub, color }: { label: string; value: string; sub: string; color: string }) {
  return (
    <Paper variant="outlined" sx={{
      ...
      <Typography variant="h4" sx={{ fontWeight: 800, color, mt: 0.25, lineHeight: 1.2, fontFamily: 'monospace' }}>
        {value}
      </Typography>
...
export default function TracesKpiRow({ traces }: Props) {
  ...
  const avgDuration = total > 0
    ? traces.reduce((s, t) => s + t.duration, 0) / total / 1000
    : 0
  const services = new Set(traces.map(t => t.service)).size

  return (
    <Box sx={{ display: 'flex', gap: 2 }}>
      <KpiCard label="TOTAL TRACES" value={total.toLocaleString()} sub="last 10 minutes" color="#a78bfa" />
      <KpiCard
        label="ERROR RATE"
      ...
      />
      <KpiCard label="AVG DURATION" value={`${avgDuration.toFixed(3)}s`} sub="mean trace duration" color="#60a5fa" />
