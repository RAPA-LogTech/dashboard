...
function DurationBar({ duration, maxDuration }: { duration: number; maxDuration: number }) {
  ...
  return (
    <Box sx={{ display: 'flex', alignItems: 'center', gap: 1, minWidth: 0 }}>
      <Typography variant="caption" sx={{ fontFamily: 'monospace', fontWeight: 700, color, fontSize: 11, whiteSpace: 'nowrap', width: 56, textAlign: 'right', flexShrink: 0 }}>
        {(duration / 1000).toFixed(3)}s
      </Typography>
      <Box sx={{ flex: 1, height: 4, bgcolor: 'action.hover', borderRadius: 1, overflow: 'hidden', minWidth: 40 }}>
        <Box sx={{ height: '100%', width: `${pct}%`, bgcolor: color, borderRadius: 1, transition: 'width 0.3s ease' }} />
      </Box>
    </Box>
  )
}

function TraceRow({ trace, maxDuration, isSelected }: { trace: Trace; maxDuration: number; isSelected: boolean }) {
  ...
  return (
    ...
    >
      {/* Timestamp */}
      <Typography variant="caption" sx={{ fontFamily: 'monospace', color: 'text.secondary', fontSize: 11, whiteSpace: 'nowrap' }}>
        {formatTimestamp(trace.startTime)}
      </Typography>

      {/* Service */}
      <Typography variant="caption" sx={{ fontWeight: 700, color: 'text.primary', fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', pr: 1 }}>
        {trace.service}
      </Typography>

      {/* Operation */}
      <Typography variant="caption" sx={{ color: 'text.secondary', fontSize: 11, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', pr: 1 }}>
        {trace.operation}
      </Typography>
      ...
      {/* Spans */}
      <Typography variant="caption" sx={{ color: 'text.secondary', fontSize: 11, textAlign: 'center' }}>
        {trace.spans.length} spans
        {errorSpans > 0 && <Box component="span" sx={{ color: '#f87171', ml: 0.5 }}>({errorSpans}✕)</Box>}
      </Typography>
      ...
      {/* Services */}
      <Box sx={{ display: 'flex', gap: 0.5, justifyContent: 'flex-end', flexWrap: 'nowrap', overflow: 'hidden' }}>
        {services.map(svc => (
          <Chip key={svc} label={svc} size="small" variant="outlined" sx={{ height: 16, fontSize: 9, '& .MuiChip-label': { px: 0.5 } }} />
        ))}
      </Box>
    </Box>
  )
...
function TracesTableComponent({ traces, selectedTraceId }: Props) {
  ...
  return (
    <Box sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1, overflow: 'visible', bgcolor: 'background.default' }}>
      {/* 헤더 */}
      <Box sx={{
        display: 'grid',
        gridTemplateColumns: '148px 110px 90px 60px 80px 1fr 180px',
